...
# def focal_loss_mc(logits, targets, gamma=2.0, alpha=None, reduction='none'):
#     """
#     Multi‑class focal loss for dense per‑pixel classification.
#     logits  : [B, C, Hc, Wc]  (raw scores, NOT softmaxed)
#     targets : [B, Hc, Wc]     (LongTensor with class indices 0 … C‑1)
#     alpha   : list/tuple with per‑class weights or None
#     """
#     # Convert to probabilities with log‑softmax -> log p_i
#     logp = F.log_softmax(logits, dim=1)               # [B,C,H,W]
#     p    = logp.exp()
#     # Gather the log‑p of the target class for every pixel
#     logpt = logp.gather(1, targets.unsqueeze(1))      # [B,1,H,W]
#     pt    = logpt.exp()                               # p_t
#     # Class weights (alpha) if provided
#     if alpha is not None:
#         at = torch.tensor(alpha, device=logits.device)[targets]  # [B,H,W]
#         logpt = logpt * at.unsqueeze(1)
#     # Focal term
#     loss = -(1 - pt) ** gamma * logpt                 # [B,1,H,W]
#     if reduction == 'none':
#         return loss.squeeze(1)                        # keep per‑pixel loss
#     elif reduction == 'mean':
#         return loss.mean()
#     else:
#         return loss.sum()
...
def focal_loss_mc(logits, targets, gamma=2.0, alpha=None, reduction='none'):
    """
    Multi‑class focal loss (dense version).
      logits  : [B, C, H, W] raw scores
      targets : [B, H, W]     class indices
      alpha   : list/tuple of per‑class weights or None
    """
    logp = F.log_softmax(logits, dim=1)          # log‑prob
    pt   = logp.exp()                            # prob
    logpt = logp.gather(1, targets.unsqueeze(1)) # log p_t
    if alpha is not None:
        at = torch.tensor(alpha, device=logits.device)[targets]  # broadcast
        logpt = logpt * at.unsqueeze(1)
    loss = -(1 - logpt.exp()) ** gamma * logpt   # focal term
    return loss.squeeze(1) if reduction == 'none' else loss.mean()



# def detector_loss(keypoint_map, logits, valid_mask=None, grid_size=8, device='cpu'):
def detector_loss(keypoint_map, logits, valid_mask=None, grid_size=8, device='cpu', loss_type='bce', gamma=2.0, alpha=0.25):
    """
    ...
    # --------  choose BCE or Focal  ------------------
    if loss_type == 'focal':
        alpha_vec = [alpha] * logits.size(1)  # same weight for all 65 classes
        ce_loss = focal_loss_mc(logits, labels, gamma=gamma, alpha=alpha_vec, reduction='none')
    else:    
    ...
    ## method 1 (now focal loss; gamma=2, alpha=[0.25 …])
    # alpha list must have C entries; put 0.25 for positives, 0.25 for dustbin
    # alpha = [0.25] * logits.size(1)
    # ce_loss = focal_loss_mc(logits, labels, gamma=2.0, alpha=alpha, reduction='none')
