...
class SyntheticShapes(Dataset):
    ...
    def batch_collator(self, samples):
        ...
        batch['raw']['img'] = torch.tensor(batch['raw']['img'][:,np.newaxis,:,:], dtype=torch.float32, device=self.device)
        batch['raw']['kpts_map'] = torch.tensor(batch['raw']['kpts_map'], dtype=torch.float32, device=self.device)
        batch['raw']['mask'] = torch.tensor(batch['raw']['mask'], dtype=torch.float32, device=self.device)
        batch['homography'] = torch.tensor(batch['homography'], dtype=torch.float32, device=self.device)
